...
final class DiagnosticsManager {
    ...
    func buildReport(selectedPlatform: CloudPlatform, targetProcessNames: [String]) -> String {
        ...
        return queue.sync {
            ...
            lines.append("Kernel network tuning: \(Preferences.presetConfig().tuneKernelNetwork)")
            lines.append("AWDL Guard: \(AWDLGuard.shared.currentStatus.rawValue)")
            lines.append("Adaptive Intelligence: \(Preferences.adaptiveIntelligenceEnabled)")
            lines.append("Session Intelligence: \(OptimizationEngine.shared.diagnosticsSummary)")
